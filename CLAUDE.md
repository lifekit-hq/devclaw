...
- **Verification fails CLOSED.** A quality-gate crash is **not** an approval — an
  ...
  **Since spec 032 (2026-09-03) the project's own verification environment is
  ...
  `mechanical:env` hold; a devclaw redeploy does NOT
  ...
  no image. It clears one of two ways: the gap **names a credential in the
  registry**, and that credential's own probe reads green — the report is
  superseded with no human verb (`specs/tiny/env-hold-defers-to-a-live-probe.md`,
  which narrows the human-only exit spec 042 made checkable); or the prose
  names nothing devclaw can probe, and `resume_goal` — which drops the
  project's worker-reported rows — is the human vouch. A brake that cannot
  observe its own release condition is the defect, not the policy: three
  goals sat for days on a token doctor read green), never the
