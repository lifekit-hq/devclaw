...
- **Repo gold standard** ([REPO-STANDARD.md](https://github.com/lifekit-hq/.github/blob/main/REPO-STANDARD.md),
  ...
  the same gates run in CI plus the `/ship` ritual and the project hooks; milestones
...
are frozen history),
the `commands/ship.md` script (the pre-PR ritual as `/ship`),
the `hooks/` directory (docs-reminder + a main-branch guard that blocks commit/push on main —
escape hatch: prefix `DEVCLAW_ALLOW_MAIN=1`), and the `skills/` directory (docs-audit,
live-shakedown).
