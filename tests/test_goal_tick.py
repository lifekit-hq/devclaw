...
# The surviving gate in tick_dispatch._dispatch_action: spec dirs exist but NONE
# is graded (no tasks.md) → held; anything else → allowed. The sibling-feature
# count check was retired (specs/tiny/slice-guard-observes-the-goal) — it
# predicted build-ahead from leftover spec dirs and parked goals behind their
# peers' live work. All use monkeypatch on speckit_feature_state_sync
# (zero-token pure-fs) so no real workspace is needed. The goal status must be
# idle (not in-flight) so the tick reaches the dispatch path.
