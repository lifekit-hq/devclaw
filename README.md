...
**Measured, not vibes.** The first pass-rate probe — real docker sandbox, real `claude`, one production .NET repo (`lifekit-dashboard`) — shipped **5/5 tickets gate-verified**: four net-new API features and one hardening fix, delivered as PRs, **+19 net-new tests, zero existing tests deleted/skipped/weakened, zero regressions**. That is a single-repo, n=5, gate-verified-**at-ship** measurement — the precursor to the formal **v0.1 proof** (10 tickets across ≥2 repos, scored *merged-without-rework* ≥6/10); direction now lives in the `Status` headers of `specs/` and the `P1`/`P2` issue labels (the 2026-07 `docs/ROADMAP.md` version-ladder was retired to git history on 2026-09-06; measurement: `evals/`). Honest scope: small-to-medium machine-verifiable backend tasks; UI and ambiguous specs still need a human.
...
| `get_scorecard_metrics(window_hours?)` | Rolling scorecard over the last N hours (default 1 week): merge rate, evaluator-verdict distribution, steer rate, first-pass hit rate, workspace breaks, and **human interventions per achieved goal** (spec 032 — the number "works without me" is measured by) — a cheap SQLite read, callable from Telegram/dashboards |
