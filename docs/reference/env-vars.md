...
| `DEVCLAW_MAX_CONCURRENT` | `4` | Global cap on concurrently-running **sandboxed tasks** — size to the host. Read once at import by `config.py`, enforced in `task_queue.py`. **Passed through by `deploy/docker-compose.devclaw.yml` (`${DEVCLAW_MAX_CONCURRENT:-4}`)** — before that line existed (added 2026-08-31) a value in `/srv/devclaw/.env` never reached the container and the cap was pinned at 4: the same silent-no-op class as the 2026-08-26 sandbox-sizing OOM incident. Set `1` for strictly serial execution — the unattended-operation mode: concurrent sandboxes contend for ONE account quota, and on 2026-08-29 four-way contention killed six tasks at `exceeded 5 usage-limit pauses` with zero delivered increments, because the pause budget is per-task but the quota is not. Host-side cognition subprocesses are NOT counted here — see `DEVCLAW_MAX_HOST_COGNITION`. |
