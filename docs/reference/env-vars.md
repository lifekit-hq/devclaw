...
| `DEVCLAW_BRANCH_STALE_THRESHOLD` | `50` | Commits-behind threshold for the hard-stale guard in `_prep_branch_target`: a pinned `target_branch` with 0 commits ahead AND commits-behind ≥ this value is refused before the engine runs. Prevents dispatching onto a branch so far behind its base that the resulting PR has massive conflict surface. |
