...
| `DEVCLAW_BRANCH_STALE_THRESHOLD` | `50` | Commits-behind threshold for the hard-stale guard, shared by BOTH the prep-time refuse (`_prep_branch_target`) and the tick-path dispatch skip (`tick_dispatch`): a branch with 0 commits ahead AND commits-behind ≥ this value is refused/skipped before the engine runs. Prevents dispatching onto a branch so far behind its base that the resulting PR has massive conflict surface. A fresh 0-ahead/0-behind goal branch is NOT stale — it dispatches (skipping on 0-ahead alone would livelock every new goal). |
