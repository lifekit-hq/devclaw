...
| `DEVCLAW_THIN_PLAN` | `0` | Migration flag (demolition P3, `docs/proposals/cognition-demolition.md`): a `long_lived` executing goal takes the THIN plan-state path — zero per-tick planner cognition; the tick mechanically dispatches "advance the goal / maintain `PLAN.md`" worker sessions and lets the grounded done-gate judge completion. Default OFF (the per-tick planner stays default) until a box `measure_passrate` re-run proves it holds the pass-rate baseline. |
