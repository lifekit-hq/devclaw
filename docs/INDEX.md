...
| [`reference/env-vars.md`](./reference/env-vars.md) | Single source of truth for every env var the runtime reads, grouped by purpose. | **CURRENT** — verified 2026-09-06 against `devclaw/config.py` + `tests/test_env_vars_doc_sync.py` green (set parity both ways, every default matches); dead cognition roles in prose (planner, scope grill, firming/decomposer, failure-analysis judge, `goal_docs`/`programs`) replaced with the live roles from `devclaw/model_tiers.py`; change history in git (`git log -- docs/reference/env-vars.md`) |
