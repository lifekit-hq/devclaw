...
|---|---|---|
| [`architecture.md`](./architecture.md) | **Start here.** Part I: the one-sitting mental model (five layers, two chains, the heartbeat, one task's journey, where state lives). Part II: the **locked contract** — per-layer contracts, the invariants (incl. grounded cognition, auto-heal), testability/replaceability, the question tree, the code map. | **CURRENT** — verified 2026-09-07 (spec 034: `project_docs` gone, worker memory = the repo's `.devclaw/`); 2026-09-06: Part I/II claims checked against the code; the gate section rewritten to match `quality/gate_policy.py` (`ALWAYS_HARD` verify/materialize/change_class/test_integrity/delivery_trust/done_gate, `DIAL_ABLE` browser/review/slice) and the `gate_pipeline` order, with the retired declared-scope gate stated once; dead references (elicitation.py, goal_docs, view_migration.py, DEVCLAW_SELF_TRIAGE, the claude_sdk engine, phases/registry, 12 pruned tests) removed; ADR 0001's agent-vs-box orthogonality folded into the layer-4 section; change history in git (`git log -- docs/architecture.md`) *Updated 2026-09-06*: one task's journey step 2 — the tick's prep is admission only; the branch rides on the action and the queue places it at run start, before the baseline capture (shared-workspace class); the admission lint's undecided-choice judge fails closed (tinyspec `eng-health-fix-now`). *Updated 2026-09-06*: the box's kernel-side fence named in the layer-4 section (tinyspec `sandbox-kernel-fence`). *Updated 2026-09-06*: the five-layers section names the import-linter contracts that enforce the layer order (tinyspec `import-contracts`). *Updated 2026-09-06 (spec 037)*: layer-3 row and the self-triage paragraph rewritten for the four retired cognition roles. *Updated 2026-09-06 (#847)*: the admission lint refuses a change in another repository and lints a pointer goal's referenced contract (tinyspec `admission-lint-pointer-goals`). *Updated 2026-09-07*: the auto-heal paragraph names the real healable set (`prep`/`env`/`ci`) and the declared `HUMAN_GATED_MECHANICAL_KINDS`; the false `corrupt_doc` self-heal claim removed and the retired slice-hold brake stated (tinyspec `slice-guard-observes-the-goal`). *Updated 2026-09-07*: a worker-reported env gap is human-gated — `instance_env_ref` is provenance, not a heal trigger; `resume_goal` is the exit (tinyspec `env-hold-observes-the-capability`, amends spec 032 US2 SC-004). *Updated 2026-09-07*: a pointer goal whose issues all closed while the gate refuses raises a Problem instead of re-dispatching (tinyspec `closed-contract-raises-a-problem`). *Updated 2026-09-07*: the run cycle is the operator's run schedule (enabled ⇒ that window, disabled ⇒ the calendar day), not a separate `DEVCLAW_RUN_CYCLE_*` knob — the drift that silently stopped the self-issue filer (tinyspec `cycle-is-when-devclaw-works`). *Updated 2026-09-07 (spec 039 US6)*: the calibration sentence in the loop-health paragraph (`intake_grades` → `goal_convergence` join, `/calibration.json`, `no_goal_armed`). *Updated 2026-09-08 (spec 042)*: the one-credential-registry bullet in the layer-4 section (`devclaw/credentials.py`, the hop set, the structural guard). *Updated 2026-09-08 (spec 041)*: the Problems paragraph — a Decision is executed by the next tick; the owner's `accept_close` closes through the shared close-and-merge tail with no gate round. *Updated 2026-09-08 (spec 041 US2)*: the auto-heal paragraph — the dispatch cap carries a Problem with a `continue` default; a fetch failure is a `mechanical:prep` hold. *Updated 2026-09-08*: the project-hold paragraph — `project_hold.next_move` is the one definition of a goal's next move, read by the holder derivation and the tick's gates; a lane-free close never queues (tinyspec `one-definition-of-runnable`).. *Updated 2026-09-09*: the problems-catalog paragraph — both read surfaces are windowed by `DEFAULT_PROBLEM_WINDOW_DAYS`, and deleted cognition roles are purged at boot rather than hidden (tinyspec `problems-catalog-recency`). *Updated 2026-09-09 (spec 039 US3+US4)*: the durable usage ledger (`usage_ledger`, written at settle and at the cognition trace, watermarked backfill), the worker usage source (the agent's own session transcript, behind the claude branch of the agent-drive seam), and cost per outcome replacing tokens-per-merged-PR. *Updated 2026-09-09 (spec 039 US5+US6)*: the console's loop-health strip and usage trend, and `goal_convergence.cost_tokens`. |

## Flows

| Doc | Purpose | Currency |
|---|---|---|
| [`flows/task-execution.md`](./flows/task-execution.md) | Temporal trace of ONE task, every hop (node 1 waiter → node 2 devclaw-mcp → node 3 ephemeral sandbox), with a "fails if" rail. | **CURRENT** — verified 2026-09-06 against `engine/sandcastle.py`, `runner/runner.py`, `engine/runner_io.py`, `task_change.py`, `quality/gate_pipeline.py`, `queue/settle.py`, `goal/remote_checks.py`, `goal/delivery_strategy.py` (gate-pipeline step, branch name, goals-dir path, env-deficiency kind fixed; the 2026-06 cascade block marked historical); change history in git (`git log -- docs/flows/task-execution.md`) *Updated 2026-09-06*: node 2's `docker run` carries the kernel-side fence (`--pids-limit 4096 --cap-drop ALL --security-opt no-new-privileges`, tinyspec `sandbox-kernel-fence`). *Updated 2026-09-07 (spec 039)*: the env-block hop files the gap through the issue doorway in the same settle and states the outcome (`#N` / the reason), and every failed attempt — including a `gh` that hangs past the wall-clock bound — leaves a `delivery/issue_filing_failed` catalog row. *Updated 2026-09-07 (tinyspec red-ci-log-to-worker)*: Step M — a red rollup steers the failing job log tail back with the check names. *Updated 2026-09-08 (spec 042)*: node 3's `acp_env` carries the payload's `agent_env` credential names — the runner forwards what the host's registry declares. *Updated 2026-09-09 (spec 039 US3)*: the runner's transcript usage source after the agent run, and node 2's third writable tmpfs that makes that transcript exist. |
...
| [`flows/autonomous-issue-pipeline.md`](./flows/autonomous-issue-pipeline.md) | End-to-end target-state flow: intake → readiness grade → dispatch → speckit scope → build → merge, with the label state machine and the human control points. | **CURRENT** — rewritten 2026-09-06: stage tags match the code (stage 1 grading exists; spec 007 PARKED; self-fix pickup gated by the human `accepted` label, no env flag; the real gate chain; merge-on-close at stage 6; spec 008 US3 label-routing NOT BUILT; live label set from `intake.py` + `self_issue.py`; spec 023 webhooks ACTIVE); change history in git (`git log -- docs/flows/autonomous-issue-pipeline.md`) *Updated 2026-09-06*: goal-path rows carry `target_branch` (= `goal/<id>`, done-check included); placement at run start, baseline re-captured per run, reused only on pause-resume. *Updated 2026-09-07*: the sibling-feature slice guard and its `mechanical:slice_hold` park are retired; only the no-graded-spec hold remains at the dispatch boundary (tinyspec `slice-guard-observes-the-goal`). |

## Reference

| Doc | Purpose | Currency |
|---|---|---|
| [`reference/devclaw-manifest.md`](./reference/devclaw-manifest.md) | The per-project `devclaw.json` manifest: fields, precedence (most-specific-wins, resolved live), the merged-base trust boundary, error posture; machine schema beside it (`devclaw-manifest.schema.json`). | **CURRENT** — verified 2026-09-06 against `devclaw/project_manifest.py`, `devclaw/env_cap.py`, `devclaw/doctor/checks_project.py` (`validation` key, `ci:definition`, the two PR-only writers, pruned test reference fixed); change history in git (`git log -- docs/reference/devclaw-manifest.md`). *Updated 2026-09-09 (tinyspec `verify-gate-is-never-narrower-than-ci`)*: `environment` gains `verifyExcludes` and is no longer declaration-only — `devclaw/goal/repo_brief.py` renders the block into the dispatch brief from the default-branch tip. |
| [`reference/env-vars.md`](./reference/env-vars.md) | Single source of truth for every env var the runtime reads, grouped by purpose. | **CURRENT** — verified 2026-09-07 (`DEVCLAW_DB` row: `project_docs` dropped by spec 034); 2026-09-06 against `devclaw/config.py` + `tests/test_env_vars_doc_sync.py` green (set parity both ways, every default matches); dead cognition roles in prose (planner, scope grill, firming/decomposer, failure-analysis judge, `goal_docs`/`programs`) replaced with the live roles from `devclaw/model_tiers.py`; change history in git (`git log -- docs/reference/env-vars.md`) *Updated 2026-09-06 (spec 037)*: `DEVCLAW_GOAL_PLAIN_SUMMARY` and the trend-detection section removed with their code. *Updated 2026-09-06*: spec 036 US1 — `DEVCLAW_COGNITION_RETRIES` now covers the SERVER_ERROR class too (a provider outage retries in-process before it reaches the fleet-wide pause). *Updated 2026-09-07 (spec 038)*: `DEVCLAW_SELF_REPO` / `DEVCLAW_SELF_ISSUE_MIN_CYCLES` rows record that a worker environment deficiency files at the hold, outside the recurrence gate. *Updated 2026-09-07*: `DEVCLAW_CI_LOG_TAIL_LINES` row. *Updated 2026-09-08 (spec 042)*: both credential rows and the refused-keys row point at `devclaw/credentials.py` as the one home. *Updated 2026-09-08*: `DEVCLAW_GIT_EMAIL` row — the runner forwards the identity into the agent env (tinyspec `worker-git-identity-and-intervention-dedup`). *Updated 2026-09-09 (tinyspec `github-credential-in-the-registry`)*: the `GITHUB_TOKEN` / `GH_TOKEN` row is replaced by a required `GH_TOKEN` row — the GitHub credential joined the registry with `repo` + `actions:read` declared, a deploy hop, and doctor's `instance.delivery.token` scope check. *Updated 2026-09-09 (tinyspec `a-torn-down-session-is-not-a-failed-dispatch`)*: the `DEVCLAW_TASK_TIMEOUT_S` row records the bounded dispatch-slot refund and the wall-clock fact now carried in the advance brief. |
...
| Doc | Purpose | Currency |
|---|---|---|
| [`runbooks/doctor.md`](./runbooks/doctor.md) | The post-redeploy checklist as one read-only, zero-LLM verb: the `doctor` MCP tool / `devclaw doctor` CLI — instance invariants (migration markers, legacy shapes, OAuth expiry, skills bundle, run-window raw key) + per-project link/workspace checks, every finding naming its remedy verb. | **CURRENT** — verified 2026-09-07 (spec 034: `project.worker_memory.health` + the `project_docs` legacy-shape check listed); 2026-09-06 against `devclaw/doctor/__init__.py`, `checks_instance.py`, `checks_project.py`, `devclaw/cli.py` (marker-module claim replaced; later check families listed); change history in git (`git log -- docs/runbooks/doctor.md`) *Updated 2026-09-07 (spec 038 US2)*: instance check `instance.env.self_repo_configured` — an unset `DEVCLAW_SELF_REPO` is a finding once the catalog holds worker-reported environment deficiencies. *Updated 2026-09-07 (spec 039 US6)*: `instance.loop_health.tables` also FAILs on a `goal_convergence` missing the calibration columns. |
