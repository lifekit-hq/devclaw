...
**Status**: SHIPPED — implemented in full 2026-09-10 in one PR (#904): US1 (span filter), US2 (`conflicting` state + one conflict routing + resume refund), US3 (accept_close outranks `auto-eval` rows). Specified and clarified with Denys the same day (one ruling, below) from the fs-431 root-cause read (13 blocks, 5 owner verbs, 2026-09-02 → 2026-09-10).
