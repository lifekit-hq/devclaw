...
- **FR-008**: ~~A merge to devclaw's main branch MUST NOT automatically rebuild-and-recreate the live VPS instance; deploy MUST be operator-triggered.~~ **AMENDED TWICE.** Spec 025 US2 (2026-08-29) allowed devclaw to deploy itself after its own merge-on-close, quiescence-gated. `specs/tiny/merge-to-main-redeploys.md` (2026-09-09) extends that to EVERY push to main: the push arms a self-deploy, the instance fires it once `count_running() == 0`. The clause's actual concern — *"a routine merge never SIGKILLs an in-flight overnight goal"* — is unchanged and is now enforced by the quiescence gate plus a named tripwire test, rather than by requiring a human to press a button.
