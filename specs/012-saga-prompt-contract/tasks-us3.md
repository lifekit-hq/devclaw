...
- [ ] **T014** [P] `tests/test_intake_readiness.py`:
  `test_sizing_assessment_spends_no_additional_cognition_call` (FR-013)
- [ ] **T015** [P] `tests/test_intake_readiness.py`:
...
- [ ] **T019** `devclaw/intake.py`: `NEEDS_SIZING_LABEL`,
  ...
  `needs_sizing` bucket.
- [ ] **T020** `devclaw/server/tools.py`: `file_intake` gains
...
- [ ] **T021** `docs/reference/intake-shape.md`: two new rows in the Fields
  table + an "Expected increments" section (claim is the filer's, grading
  validates, disagreement surfaces, shape is always a saga).
- [ ] **T022** `docs/flows/autonomous-issue-pipeline.md`: the grade stage
  records two axes and can emit `needs-sizing`.
- [ ] **T023** `docs/INDEX.md`: currency tags for both changed docs.
- [ ] **T024** `ruff check .` clean; full suite at or above the 2075-passed
