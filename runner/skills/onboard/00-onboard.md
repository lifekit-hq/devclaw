...
## The verification environment — `.github/workflows/verify.yml`

The project's own CI is its verification environment (spec 032): devclaw reads the CI result for every delivered PR head as a fact, and a project with no CI definition is not dispatchable. Write the workflow as a sibling of the Dockerfile:

- **Create it ONLY when `.github/workflows/` has no workflow at all.** An existing CI definition is the source of truth — never rewrite or add a second one.
- **One job that runs the manifest's `verifyCmd`** (from `devclaw.json`, or the gate command you named in AGENTS.md) on `push` and `pull_request`, with the services that command needs (a database service block when the tests need one) and the SDK setup steps for the stack.
- Name the job plainly (`verify`), keep it under ~40 lines, and put the same DRAFT marker comment at the top.
